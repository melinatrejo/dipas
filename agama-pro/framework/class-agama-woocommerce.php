...
if( class_exists( 'Woocommerce' ) ):
	class WooCommerce {
		...
		function agama_wrapper_start() {
            ...
            $single_product = esc_attr( get_theme_mod( 'agama_wc_single_products_sidebar', false ) );
			if( ! is_active_sidebar( 'agama-shop-sidebar' ) || ! $single_product && is_product()  ) {
				$class = 'col-md-12';
			} else {
				$class = 'col-md-9';
			}
		...
		function shop_sidebar() {
            ...
            $single_product = esc_attr( get_theme_mod( 'agama_wc_single_products_sidebar', false ) );
			if( 
                is_active_sidebar( 'agama-shop-sidebar' ) && ! is_product() || 
                is_active_sidebar( 'agama-shop-sidebar' ) && $single_product && is_product() 
            ) {
        ...
        function ajax_product_remove() {
            ...
            $data = array(
                'fragments' => apply_filters( 'woocommerce_add_to_cart_fragments', array(
                        '.agama-cart-content' => $cart_content
                    )
                ),
		...
		function header_cart_counter_fragment( $fragments ) {
            ...
            <div class="cart_count">
                <?php echo sprintf( 
                    _n( '%d', '%d', $woocommerce->cart->cart_contents_count, 'agama-pro' ), 
        ...
        function header_cart_dropdown_fragment( $fragments ) {
            ...
            echo Helper::get_wc_cart_contents();
            
            $fragments['.agama-cart-content'] = ob_get_clean();
		...
		function products_filter() {
			...
			$product_order['default'] 	 = esc_html__( 'Default Order', 'agama-pro' );
			$product_order['title'] 	 = esc_html__( 'Name', 'agama-pro' );
			$product_order['price'] 	 = esc_html__( 'Price', 'agama-pro' );
			$product_order['date'] 		 = esc_html__( 'Date', 'agama-pro' );
			$product_order['popularity'] = esc_html__( 'Popularity', 'agama-pro' );

			$product_sort['asc'] 		 = esc_html__( 'Click to order products ascending', 'agama-pro' );
			$product_sort['desc'] 		 = esc_html__( 'Click to order products descending', 'agama-pro' );

			$per_page_string 		 	 = esc_html__( 'Products per page', 'agama-pro' );
			...
			$output  = '<div class="product_filtering clearfix">';
				...
				$output .= '<ul class="filter_param filter_param_order" onclick="">';
					$output .= '<li><span class="current_li">' . esc_html__( 'Sort by', 'agama-pro' ) . ': <strong>' . $product_order[$order] . '</strong></span>';
						$output .= '<ul>';
		...
		function add_cart_button() {
			...
			if( $output && ! post_password_required() ) echo '<div class="custom_cart_button '. $extraClass .' clearfix">' . $output . '</div>';
		}
